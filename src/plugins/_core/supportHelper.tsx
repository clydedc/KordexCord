...
import { platformName } from "@kordexcordplugins/kordexcordHelper/utils";
import { gitHash, gitHashShort } from "@shared/vencordUserAgent";
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, KORDEXBOP_CONTRIB_ROLE_ID, KORDEXCORD_TEAM, GUILD_ID, SUPPORT_CHANNEL_ID, SUPPORT_CHANNEL_IDS, VC_CONTRIB_ROLE_ID, VC_DONOR_ROLE_ID, VC_GUILD_ID, VC_REGULAR_ROLE_ID, VC_SUPPORT_CHANNEL_IDS, VENCORD_CONTRIB_ROLE_ID } from "@utils/constants";
import { sendMessage } from "@utils/discord";
import { Logger } from "@utils/Logger";
import { Margins } from "@utils/margins";
import { isAnyPluginDev, isKordexcordGuild, isKordexcordSupport, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    KORDEXCORD_TEAM, // Kordexcord Team
    DONOR_ROLE_ID, // Kordexcord Donor
    CONTRIB_ROLE_ID, // Kordexcord Contributor
    KORDEXBOP_CONTRIB_ROLE_ID, // Kordexbop Contributor
...
async function generateDebugInfoMessage() {
    ...
    const client = (() => {
        ...
        if (IS_KORDEXBOP) {
            ...
            if (equibopGitHash) {
                ...
                return `Kordexbop v${VesktopNative.app.getVersion()} • [${shortHash}](<https://github.com/Kordexcord/Kordexbop/commit/${equibopGitHash}>)`;
            }
            return `Kordexbop v${VesktopNative.app.getVersion()}`;
        }
    ...
    const info = {
        Kordexcord:
            `v${VERSION} • [${gitHashShort}](<https://github.com/Kordexcord/Kordexcord/commit/${gitHash}>)` +
            `${IS_KORDEXBOP ? "" : SettingsPlugin.getVersionInfo()} - ${Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(BUILD_TIMESTAMP)}`,
    ...
    const commonIssues = {
        ...
        "Kordexcord DevBuild": !IS_STANDALONE,
        "Kordexbop DevBuild": IS_KORDEXBOP && tryOrElse(() => VesktopNative.app.isDevBuild?.(), false),
...
export default definePlugin({
    ...
    commands: [
        {
            name: "Kordexcord-debug",
            description: "Send Kordexcord debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isKordexcordGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "Kordexcord-plugins",
            description: "Send Kordexcord plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isKordexcordGuild(ctx?.guild?.id, true),
            execute: () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (VC_SUPPORT_CHANNEL_IDS.includes(channelId) && Vencord.Plugins.isPluginEnabled("VCSupport") && !clicked) {
                return Alerts.show({
                    ...
                    body: <div>
                        ...
                        <Paragraph>issue could be caused by Kordexcord!</Paragraph>
                    </div>,
                    confirmText: "Go to Kordexcord Support",
                    onConfirm() {
                        clicked = true;
                        VencordNative.native.openExternal("https://Kordexcord.org/discord");
                    },
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        title: "Hold on!",
                        body: <div>
                            <Paragraph>You are using an outdated version of Kordexcord! Chances are, your issue is already fixed.</Paragraph>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using an externally updated Kordexcord version, the ability to help you here may be limited.</Paragraph>
                        <Paragraph className={Margins.top8}>
                            Please join the <Link href="https://Kordexcord.org/discord">Kordexcord Server</Link> for support,
            ...
            if (!IS_STANDALONE && !settings.store.dismissedDevBuildWarning) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using a custom build of Kordexcord, which we do not provide support for!</Paragraph>

                        <Paragraph className={Margins.top8}>
                            We only provide support for <Link href="https://github.com/Kordexcord/Kordexcord">official builds</Link>.
                            Either <Link href="https://github.com/KordexCord/Equilotl">switch to an official build</Link> or figure your issue out yourself.
                        </Paragraph>
    ...
    renderMessageAccessory(props) {
        ...
        const equicordSupport = isKordexcordSupport(props.message.author.id);
        ...
        if (isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel) && equicordSupport) {
            if (props.message.content.includes("/Kordexcord-debug") || props.message.content.includes("/Kordexcord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Kordexcord & Vencord plugin developers for support!
                <br />
