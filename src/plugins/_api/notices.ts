    ...
    patches: [
        {
            ...
            replacement: [
                ...
                // FIXME(Bundler minifier change related): Remove the non used compability once enough time has passed
                {
                    ...
                    replace: (m, notice) => `if(${notice}?.id=="KordexcordNotice")return(${notice}=null,Vencord.Api.Notices.nextNotice(),true);${m}`,
                    noWarn: true,
                },
                {
                    match: /(?<=function (\i)\(\i\){)return null!=(\i)(?=.+?NOTICE_DISMISS:\1)/,
                    replace: (m, _, notice) => `if(${notice}?.id=="KordexcordNotice")return(${notice}=null,Vencord.Api.Notices.nextNotice(),true);${m}`
                }
            ]
        }
    ],
