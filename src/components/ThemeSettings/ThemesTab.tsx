...
export function CspErrorCard() {
    ...
    const allowUrl = async (url: string) => {
        ...
        const result = await VencordNative.csp.requestAddOverride(baseUrl, ["connect-src", "img-src", "style-src", "font-src"], "Kordexcord Themes");
        if (result !== "ok") return;
    ...
    return (
        <ErrorCard className="vc-settings-card">
            ...
            <Paragraph>
                After allowing a domain, you have to fully close (from tray / task manager) and restart {IS_DISCORD_DESKTOP ? "Discord" : IS_KORDEXBOP ? "Kordexbop" : "Vesktop"} to apply the change.
            </Paragraph>
