...
import { KordexcordDevsById, VencordDevsById } from "@utils/constants";
...
type PluginFilter = "all" | "Kordexcord" | "vencord" | "modified" | "user";
...
function ContributorModal({ user }: { user: User; }) {
    ...
    const plugins = useMemo(() => {
        ...
        const pluginsByAuthor = (VencordDevsById[user.id] || KordexcordDevsById[user.id])
            ? allPlugins.filter(p => p.authors.includes(VencordDevsById[user.id] || KordexcordDevsById[user.id]))
            : allPlugins.filter(p =>
    ...
    const ContributedHyperLink = <Link href="https://github.com/Kordexcord/Kordexcord">contributed</Link>;
