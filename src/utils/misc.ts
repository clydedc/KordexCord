...
import { KORDEXCORD_HELPERS, KordexcordDevsById, GUILD_ID, SUPPORT_CHANNEL_ID, VencordDevsById } from "./constants";
...
export const isKordexcordPluginDev = (id: string) => Object.hasOwn(KordexcordDevsById, id);
export const shouldShowKordexcordContributorBadge = (id: string) => isKordexcordPluginDev(id) && KordexcordDevsById[id].badge !== false;

export const isAnyPluginDev = (id: string) => Object.hasOwn(VencordDevsById, id) || Object.hasOwn(KordexcordDevsById, id);
...
export function isKordexcordGuild(id: string | null | undefined, isGuildId: boolean = false): boolean {
    if (!id) return false;
...
export function isKordexcordSupport(userId: string | null | undefined): boolean {
    if (!userId) return false;

    const member = GuildMemberStore.getMember(GUILD_ID, userId);
    return member?.roles?.includes(KORDEXCORD_HELPERS) || false;
}
