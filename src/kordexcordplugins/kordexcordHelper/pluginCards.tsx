...
import { KORDEXBOT_USER_ID } from "@utils/constants";
import { isKordexcordGuild, isKordexcordSupport } from "@utils/misc";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Kordexcord. Try updating!";
    ...
    if (required) {
        const tooltipText = p.required || !dependents.length
            ? "This plugin is required for Kordexcord to function."
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    message.embeds?.forEach(embed => {
        if (!embed.url?.startsWith("https://Kordexcord.org/plugins/") && !embed.url?.startsWith("https://vencord.dev/plugins/")) return;

        const isKordexcord = isKordexcordGuild(message.channel_id) && isKordexcordSupport(message.author.id);
        if (!isKordexcord) return;
    ...
    if (message.author.id === KORDEXBOT_USER_ID && components?.length >= 4) {
        const description = components[1]?.content;
        const pluginUrl = components.find((c: any) => c?.components)?.components[0]?.url;
        if (pluginUrl?.startsWith("https://Kordexcord.org/plugins/") || pluginUrl?.startsWith("https://vencord.dev/plugins/")) {
