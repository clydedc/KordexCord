...
import { ChromiumIcon, DevBannerIcon, DiscordIcon, ElectronIcon, KordexcordIcon } from "@kordexcordplugins/discordDevBanner/icons";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const replaced = baseFormat
        ...
        .replace(/{equicordName}/g, "Kordexcord")
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{equicordIcon}":
                return <span key={`icon-Kordexcord-${i}`} className="vc-discord-dev-banner-icons"><KordexcordIcon /></span>;
            case "{electronIcon}":
