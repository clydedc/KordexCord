...
import { Devs, KordexcordDevs } from "@utils/constants";
import { isKordexcordPluginDev, isPluginDev } from "@utils/misc";
...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "EquicordDonor":
            return (
                <span style={{ order: settings.store.EquicordDonorPosition }}>
                    {badges.getKordexcordDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
        ...
        case "EquicordContributer":
            return isKordexcordPluginDev(author.id) ? (
                <span style={{ order: settings.store.EquicordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIconClassName}
                        name="Kordexcord Contributor"
                        size={20}
                        src={"https://Kordexcord.org/assets/favicon.png"}
                    />
                </span>
            ) : null;
        case "VencordDonor":
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, KordexcordDevs.KrystalSkull],
