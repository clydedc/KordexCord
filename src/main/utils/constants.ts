...
export const DATA_DIR = process.env.KORDEXCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "KordexcordData", suffix)
        : join(app.getPath("userData"), "..", "Kordexcord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Kordexcord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
