...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("Kordexcord", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("Kordexcord://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Kordexcord] Installed React Developer Tools"))
                    .catch(err => console.error("[Kordexcord] Failed to install React Developer Tools", err));
        } catch { }
