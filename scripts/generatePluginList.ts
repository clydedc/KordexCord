...
async function parseFile(fileName: string) {
    ...
    for (const node of file.getChildAt(0).getChildren()) {
        ...
        if (target) {
            if (!["web", "discordDesktop", "vesktop", "kordexbop", "desktop", "dev"].includes(target)) throw fail(`invalid target ${target}`);
...
(async () => {
    ...
    await Promise.all(["src/plugins", "src/plugins/_core", "src/kordexcordplugins"].flatMap(dir =>
