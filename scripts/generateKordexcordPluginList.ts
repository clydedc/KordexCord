...
interface PluginData {
    ...
    target: "discordDesktop" | "vesktop" | "kordexbop" | "desktop" | "web" | "dev";
    filePath: string;
    dirName: string;
    isModified: boolean;
}
...
const kordexcordDevs = {} as Record<string, Dev>;
...
function parseKordexcordDevs() {
    const file = createSourceFile("constants.ts", readFileSync("src/utils/constants.ts", "utf8"), ScriptTarget.Latest);

    for (const child of file.getChildAt(0).getChildren()) {
        if (!isVariableStatement(child)) continue;

        const devsDeclaration = child.declarationList.declarations.find(d => hasName(d, "KordexcordDevs"));
        if (!devsDeclaration?.initializer || !isCallExpression(devsDeclaration.initializer)) continue;

        const value = devsDeclaration.initializer.arguments[0];

        if (!isSatisfiesExpression(value) || !isObjectLiteralExpression(value.expression)) throw new Error("Failed to parse KordexcordDevs: not an object literal");

        for (const prop of value.expression.properties) {
            const name = (prop.name as Identifier).text;
            const value = isPropertyAssignment(prop) ? prop.initializer : prop;

            if (!isObjectLiteralExpression(value)) throw new Error(`Failed to parse KordexcordDevs: ${name} is not an object literal`);
    ...
    throw new Error("Could not find KordexcordDevs constant");
}
...
async function parseFile(fileName: string) {
    ...
    for (const node of file.getChildAt(0).getChildren()) {
            ...
            switch (key) {
                ...
                case "authors":
                    ...
                    data.authors = value.elements.map(e => {
                        ...
                        const d = devs[getName(e)!] || kordexcordDevs[getName(e)!];
        ...
        if (target) {
            if (!["web", "discordDesktop", "vencordDesktop", "kordexbop", "desktop", "dev"].includes(target)) throw fail(`invalid target ${target}`);
...
(async () => {
    ...
    await Promise.all(["src/kordexcordplugins"].flatMap(dir =>
