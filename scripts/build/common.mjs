...
function getGitHash() {
    if (process.env.KORDEXCORD_HASH) return process.env.KORDEXCORD_HASH.trim();
    try {
...
/**
 * @type {(kind: "web" | "discordDesktop" | "vesktop" | "kordexbop") => import("esbuild").Plugin}
 */
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "kordexcordplugins", "kordexcordplugins/_api"];
            let code = "";
            let pluginsCode = "\n";
            let metaCode = "\n";
            let excludedCode = "\n";
                ...
                for (const file of files) {
                    ...
                    if (target && !IS_REPORTER) {
                        const excluded =
                            ...
                            (target === "vesktop" && kind !== "vesktop" && kind !== "kordexbop") ||
                            (target === "kordexbop" && kind !== "kordexbop" && kind !== "vesktop");
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.KORDEXCORD_REMOTE;
            if (!remote) {
                try {
