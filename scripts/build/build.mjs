...
const globNativesPlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-natives" }, async () => {
            const pluginDirs = ["plugins", "userplugins", "kordexcordplugins"];
            let code = "";
            let natives = "\n";
            let i = 0;
            /**
...
const buildConfigs = ([
    ...
    // Vencord Desktop main & renderer & preload
    {
        ...nodeCommonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/main/index.ts")],
        outfile: "dist/kordexbop/main.js",
    ...
    },
    {
        ...commonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/Vencord.ts")],
        outfile: "dist/kordexbop/renderer.js",
        format: "iife",
        target: ["esnext"],
    ...
    },
    {
        ...nodeCommonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/preload.ts")],
        outfile: "dist/kordexbop/preload.js",
        footer: { js: "//# sourceURL=file:///VencordPreload\n" + sourceMapFooter("preload") },
        sourcemap,
        define: {
            ...defines,
...
await Promise.all([
    ...
    writeFile("dist/kordexbop/package.json", JSON.stringify({
        name: "kordexcord",
        main: "main.js"
    }))
]);
...
await Promise.all([
    ...
    createPackage("dist/kordexbop", "dist/kordexbop.asar"),
]);
